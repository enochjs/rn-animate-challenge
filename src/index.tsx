...
import { StatusBar, SafeAreaView, SectionList, Text, View, StyleSheet, FlatList, Dimensions } from 'react-native';
...
export default function App() {
  ...
  const balanceStylez = useAnimatedStyle(() => {
    ...
    return {
      opacity: interpolate(headerAnim.value, [0, _itemSize, _itemSize + _extraSectionHeaderSpacing, headerHeight.value], [1, 1, 1, 0]),
      transform: [
        {
          translateY: interpolate(
            headerAnim.value,
            [0, _itemSize, _itemSize + _extraSectionHeaderSpacing, _itemSize + _extraSectionHeaderSpacing + 1],
            [0, 0, 0, -1]
          ),
        },
      ],
  ...
  const headerStylez = useAnimatedStyle(() => {
    return {
      transform: [
        ...
        {
          translateY: interpolate(headerAnim.value, [0, _itemSize], [0, -_itemSize / 2], Extrapolate.CLAMP),
        },
        {
          rotateX: `${interpolate(headerAnim.value, [0, _itemSize], [0, 90], Extrapolate.CLAMP)}deg`,
        },
      ],
      opacity: interpolate(headerAnim.value, [0, _itemSize / 2, _itemSize], [1, 1, 0], Extrapolate.CLAMP),
    };
  });
  // React.useEffect(() => {
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <Animated.View style={[{ padding: _spacing, marginBottom: _spacing }, balanceStylez]}>
          <Text style={[styles.regular, { fontSize: 42, color: _colors.text }]}>{faker.finance.amount(500, 8500, 0, '$')}</Text>
          ...
          renderItem={({ item, index }) => {
            return (
              ...
              >
                ...
                <Text style={[styles.bold, { fontSize: 24, color: _colors.text }]}>{item.amount}</Text>
                <Text style={[styles.regular, { fontSize: 16, opacity: 0.6, color: _colors.text }]}>{item.department}</Text>
              </View>
            );
          }}
        />
          ...
          return (
            ...
            >
              ...
              <View style={{ paddingHorizontal: _spacing }}>
                ...
                <Text style={[styles.regular, { color: _colors.text, opacity: 0.6 }]}>{item.department}</Text>
              </View>
              <View
                style={{
                  flexGrow: 1,
