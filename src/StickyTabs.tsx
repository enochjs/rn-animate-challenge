...
const tabs = ['Today', 'Fri 18', 'Sat 19', 'Sun 20', 'Mon 21', 'Tue 22', 'Wed 23', 'Thu 24', 'Fri 25', 'Sat 26', 'Sun 27'];
const Tabs = ({ scrollY }) => {
  const stylez = useAnimatedStyle(() => {
    return {
  ...
  const navTextStyle = useAnimatedStyle(() => {
    return {
      ...
      transform: [
        {
          translateY: interpolate(scrollY.value, inputRange, [0, -_headerHeightShrink / 2], Extrapolate.CLAMP),
        },
        {
  ...
  return (
    <Animated.View style={[{ backgroundColor: colors.tab, justifyContent: 'center', paddingLeft: 50 }, stylez]}>
      ...
      <Animated.ScrollView horizontal showsHorizontalScrollIndicator={false} style={[{ flexGrow: 0 }, scrollStyles]}>
        {tabs.map((tab, index) => (
          <View key={tab} style={{ marginRight: 40 }}>
            <Text style={{ color: colors.tabText, fontWeight: '700', fontSize: 16 }}>{tab}</Text>
            {index === 0 && <View style={{ marginTop: 5, height: 2, backgroundColor: colors.tabText, width: '50%' }} />}
          </View>
        ))}
      </Animated.ScrollView>
    </Animated.View>
    ...
    <View style={{ flex: 1 }}>
      ...
      >
        {/* <Animated.View
            ...
            /> */}
        <Text style={{ marginBottom: 20, lineHeight: 22 }}>{faker.lorem.sentences(faker.number.int(10) + 2)}</Text>
        <Text style={{ marginBottom: 20, lineHeight: 22 }}>{faker.lorem.sentences(faker.number.int(10) + 2)}</Text>
        <Text style={{ marginBottom: 20, lineHeight: 22 }}>{faker.lorem.sentences(faker.number.int(10) + 2)}</Text>
        <Text style={{ marginBottom: 20, lineHeight: 22 }}>{faker.lorem.sentences(faker.number.int(10) + 2)}</Text>
        <Text style={{ marginBottom: 20, lineHeight: 22 }}>{faker.lorem.sentences(faker.number.int(10) + 2)}</Text>
