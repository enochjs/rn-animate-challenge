...
const _heightOffset = height * 0.25;

export function WanderAppInteractions() {
  ...
  return (
    ...
    >
      ...
      <View style={{ width, height, position: 'absolute', top: 0, left: 0, overflow: 'hidden' }}>
        <Animated.Image ref={ref} source={{ uri: _image }} style={{ flex: 1, opacity, transform: [{ scale }] }} />
      </View>
      <View style={{ padding: 20, minHeight: height }}>
        <View style={{ alignItems: 'center', height: _heightOffset, justifyContent: 'center' }}>
          <Animated.Text
            style={{ fontSize: 12, textTransform: 'uppercase', fontWeight: '800', letterSpacing: 4, marginBottom: 10, color: 'white', opacity }}
            numberOfLines={1}
          ...
          <View>
            <Animated.Text
              style={{ fontSize: 42, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 14, color: 'white', opacity }}
              numberOfLines={1}
              adjustsFontSizeToFit
            >
              farmers camp
        ...
        {[...Array(10).keys()].map((index) => (
          <MText
            ...
            style={{ marginBottom: 20, fontSize: 16, lineHeight: 24, fontFamily: 'Georgia' }}
          >
            {faker.lorem.sentences(faker.number.int(3) + 2)}
          </MText>
        ))}
      </View>
    </Animated.ScrollView>
