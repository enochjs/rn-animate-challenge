...
import Animated, { useSharedValue, useAnimatedStyle, useDerivedValue, withTiming, cancelAnimation, runOnJS } from 'react-native-reanimated';
...
export function Zoom(props: Props) {
  ...
  // is end
  const isPanEndOne = useSharedValue(false);
  //
    ...
    const doubleTap = Gesture.Tap()
      .onStart((e) => {
        ...
        if (scale.value !== 1) {
          ...
          translationX.value = withTiming(-1 * (maximumZoomScale * (e.x - viewWidth.value / 2)));
          translationY.value = withTiming(-1 * (maximumZoomScale * (e.y - viewHeight.value / 2)));
        }
      })
      .numberOfTaps(2);

    if (zoomListContext?.simultaneousPanGestureRef) {
      pan.simultaneousWithExternalGesture(zoomListContext?.simultaneousPanGestureRef);
    }

    return Gesture.Race(doubleTap, simultaneousGesture ? Gesture.Simultaneous(pan, pinch, simultaneousGesture) : Gesture.Simultaneous(pan, pinch));
  ...
  return (
    <GestureDetector gesture={gesture}>
      <Animated.View {...props} onLayout={memoizedOnLayout} style={memoizedStyle} />
    </GestureDetector>
  );
}
