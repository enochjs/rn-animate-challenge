...
export default function FixedColumns<T extends IDefaultData>(props: IFixedColumnsProps<T>) {
  const { data, columns, heightArr, widthArr, flexArr, scrollY, top, fixed = 'left' } = props;
    ...
    >
      <Animated.View style={[animatedYStyle]}>
        <View className="flex-row">
          {columns.map((c, index) => (
            <Col key={c.dataIndex as string} column={c as any} width={widthArr?.[index]} flex={flexArr?.[index]} data={data} heightArr={heightArr} />
          ))}
        </View>
      </Animated.View>
    </View>
