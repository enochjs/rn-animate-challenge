...
function TarotCard({ card, cardIndex, index }: { card: TarotCard; index: SharedValue<number>; cardIndex: number }) {
  ...
  const stylez = useAnimatedStyle(() => {
    return {
      transform: [
        ...
        {
          translateY: interpolate(index.value, [cardIndex - 1, cardIndex, cardIndex + 1], [0, -tarotCardSize.height / 2, 0], Extrapolate.CLAMP),
        },
      ],
    };
  });
  return (
    ...
    >
      <Image key={card.key} source={{ uri: card.uri }} style={styles.tarotCardBackImage} />
    </Animated.View>
  );
}

function TarotWheel({ cards, onCardChange }: { cards: TarotCard[]; onCardChange: (cardIndex: number) => void }) {
    ...
    <GestureDetector gesture={pan}>
      ...
      >
        {cards.map((card, cardIndex) => {
          return <TarotCard card={card} key={card.key} index={interpolatedIndex} cardIndex={cardIndex} />;
        })}
      </Animated.View>
    </GestureDetector>
  );
  ...
  return (
    <GestureHandlerRootView style={{ flex: 1 }}>
      ...
      >
        ...
        <TarotWheel cards={tarotCards} onCardChange={(cardIndex) => setActiveCardIndex(cardIndex)} />
      </View>
